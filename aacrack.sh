...
# Low-end iDevice (with low memory) (doesn't really help, though)
RClowend="NO"
...
##			ExtrasAslr=" ASLR"
			...
			# Workaround for non-working posix_spawn under iOS6
...
###																																							LowEndFunction
				...
				if [ "$AllonsSurLaRoute" = "" ]; then
					echo "Your 'gdb' failed. Please, update it."
					rm -fr "$WorkDir"
					return 1
				fi
			fi
...
if [ $NewFields -ne "11" -a $NewFields -ne "7" ]; then
	echo "${Meter70}${escYellow}$MsgWarning: $MsgWrnMeta ?${escReset}"
	#BEGIN DEBUG
	#echo "( $NewFields )"
	#cat /tmp/diff.txt
	#END DEBUG
...
echo "${Meter0}*** ${escUnder}Rasticrac v3.0.1${escReset} ***"
